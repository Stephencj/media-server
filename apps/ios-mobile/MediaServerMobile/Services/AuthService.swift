...
class AuthService: ObservableObject {
    ...
    /// Authorizes a browser web-login session by sending the scanned code
    /// to the server. Caller is responsible for showing UI feedback.
    func claimWebLoginCode(_ code: String) async throws {
        guard let url = URL(string: Secrets.serverURL + "/api/auth/web-login/claim") else {
            throw APIError.invalidURL
        }
        guard let bearer = self.token else { throw APIError.unauthorized }
        ...
        let message = String(data: data, encoding: .utf8) ?? "Claim failed"
